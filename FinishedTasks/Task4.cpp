...
BC barycentric(int Pi, int Px, Triangle tri);
...
int main() {
	...
	*/
	...
	string outfile = "bunny_task4.png";
	int height = 512;
	int width = 512;
	int task = 4;
	...
	for (int i = 0; i < triangles.size(); ++i) {
		...
		for (int y = tri.ymin; y < tri.ymax; ++y) {
			for (int x = tri.xmin; x < tri.xmax; ++x) {
				...
				// If in triangle then compute color and write pixel to image
				BC abc = barycentric(x, y, tri);
				if (abc.a >= 0 && abc.b >= 0 && abc.c >= 0) {
					// Alpha is percentage of y from ymin and y max (translated)
					double alpha = (y - (scale * ymin + ty)) / ((scale * ymax + ty) - (scale * ymin + ty));
					r = 255 * alpha;
					g = 0;
					b = 255 * (1 - alpha);
...
BC barycentric(int Px, int Py, Triangle tri) {
	// How to go CCW?
